...
for scenario in repository search questions; do
  ...
  docker run --rm --network "$network" \
    ...
    -e PERFORMANCE_REVIEW_ID="${PERFORMANCE_REVIEW_ID:-}" \
    -e PERFORMANCE_WORKFLOW_DECISION="${PERFORMANCE_WORKFLOW_DECISION:-approve}" \
    -e PERFORMANCE_WORKFLOW_IDEMPOTENCY_KEY="${PERFORMANCE_WORKFLOW_IDEMPOTENCY_KEY:-}" \
    -e PERFORMANCE_SKIP_QUESTIONS="${PERFORMANCE_SKIP_QUESTIONS:-false}" \
