...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint("agreement_id", "agreement_version_id", "idempotency_key", name="uq_review_cases_idempotency"),
    )
    op.create_index("ix_review_cases_scope_created", "review_cases", ["organization_id", "workspace_id", "created_at"])
    op.create_table(
        "review_assignments",
        sa.Column("id", sa.Uuid(), nullable=False),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("review_id", "idempotency_key", name="uq_review_assignments_idempotency"),
    )
    op.create_index("ix_review_assignments_inbox", "review_assignments", ["organization_id", "workspace_id", "assignee_id", "status"])
    op.create_table(
        "review_comments",
        sa.Column("id", sa.Uuid(), nullable=False),
        sa.Column("review_id", sa.Uuid(), nullable=False),
    ...
    op.create_index("ix_review_comments_scope_review", "review_comments", ["organization_id", "workspace_id", "review_id"])
    op.create_table(
    ...
    op.create_index("ix_review_notification_events_pending", "review_notification_events", ["delivered_at", "created_at"])
