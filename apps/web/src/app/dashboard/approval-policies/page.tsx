...
export default async function ApprovalPoliciesPage() {
  if (!(await auth())?.user)
    redirect("/sign-in?callbackUrl=%2Fdashboard%2Fapproval-policies");
  const configuredScope = scope();
  if (!configuredScope)
    return (
      <main className="mx-auto max-w-7xl px-6 py-10">
        <p role="alert">Approval policy administration is not configured.</p>
      </main>
    );
  const token = await getKeycloakAccessToken(await headers());
  let policies;
  try {
    policies = await listApprovalPolicies({ scope: configuredScope, token });
  } catch {
    return (
      <main className="mx-auto max-w-7xl space-y-6 px-6 py-10">
        ...
        <p
          ...
          role="alert"
        >
          Unable to load approval policies. Check your access and try again.
        </p>
      </main>
    );
  }
