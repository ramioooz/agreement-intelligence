...
export async function POST(
  ...
  const response = await fetch(
    `${apiBaseUrl.replace(/\/$/, "")}/reviews/${encodeURIComponent(reviewId)}/comments?organization_id=${encodeURIComponent(process.env.API_ORGANIZATION_ID ?? "")}&workspace_id=${encodeURIComponent(process.env.API_WORKSPACE_ID ?? "")}`,
    {
      method: "POST",
      cache: "no-store",
      headers: {
