...
export async function GET(
  ...
  if (artifact !== "pdf" && artifact !== "manifest") return Response.json({ message: "Not found." }, { status: 404 });
  const response = await fetch(`${apiBaseUrl.replace(/\/$/, "")}/reviews/${encodeURIComponent(reviewId)}/final-package/${artifact}`, {
    cache: "no-store",
